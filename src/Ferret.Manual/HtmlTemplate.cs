...
public static class HtmlTemplate
{
    ...
    private const string Css = """
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        html.dark{--bg:#181825;--fg:#cdd6f4;--code-bg:#313244;--border:#45475a}
        ...
        .sidebar-header{padding:1rem;border-bottom:1px solid #313244}
        .sidebar-top{display:flex;align-items:center;justify-content:space-between;margin-bottom:.75rem}
        .logo{color:var(--sidebar-fg);text-decoration:none;font-size:1rem;font-weight:600}
        .theme-toggle{background:none;border:1px solid #45475a;border-radius:5px;
                      color:var(--sidebar-fg);cursor:pointer;font-size:.85rem;
                      padding:.15rem .45rem;line-height:1.5;transition:border-color .15s}
        ...
        .content{flex:1;min-width:0;display:flex;flex-direction:column;overflow-y:auto}
        .prose{padding:2.5rem 3rem;flex:1;width:100%}
        ...
        html.dark .prose blockquote{color:#a6adc8}
        ...
        html.dark .prose tr:nth-child(even) td{background:#1f1f2e}
        .page-footer{padding:.45rem 3rem;border-top:1px solid var(--border);
                     display:flex;align-items:center;justify-content:space-between;
                     gap:.75rem;flex-wrap:wrap;flex-shrink:0}
        .page-nav-link{color:var(--accent);text-decoration:none;font-size:.85rem;
                       font-weight:500;white-space:nowrap}
        .page-nav-link:hover{text-decoration:underline}
        .footer-meta{display:flex;align-items:center;gap:.15rem;font-size:.78rem;
                     color:#999;flex-wrap:wrap;justify-content:center}
        .footer-meta a{color:#999}
        .footer-meta a:hover{color:var(--accent)}
        .footer-meta .sep{color:#ccc;margin:0 .15rem}
        html.dark .footer-meta{color:#6c7086}
        html.dark .footer-meta a{color:#6c7086}
        html.dark .footer-meta .sep{color:#45475a}
        .sidebar::-webkit-scrollbar{width:5px}
        .sidebar::-webkit-scrollbar-track{background:transparent}
        .sidebar::-webkit-scrollbar-thumb{background:#45475a;border-radius:3px}
        .sidebar::-webkit-scrollbar-thumb:hover{background:#6c7086}
        .sidebar{scrollbar-width:thin;scrollbar-color:#45475a transparent}
        ...
        .content{scrollbar-width:thin;scrollbar-color:#d1d5db transparent}
        html.dark .content::-webkit-scrollbar-thumb{background:#45475a}
        html.dark .content::-webkit-scrollbar-thumb:hover{background:#6c7086}
        html.dark .content{scrollbar-color:#45475a transparent}
        @media(max-width:900px){.sidebar{width:240px}.prose{padding:2rem}.page-footer{padding:.45rem 2rem}}
        @media(max-width:640px){.sidebar{display:none}.prose{padding:1.5rem 1rem}.page-footer{padding:.45rem 1rem}}
        """;

    private const string LunrJs = """
/**
...
""";
    ...
    private const string ThemeScript = """
        (function(){
          ...
          var stored=localStorage.getItem('ferret-theme');
          if(stored==='dark'){document.documentElement.classList.add('dark');btn.textContent='☀';}
          btn.addEventListener('click',function(){
            var dark=document.documentElement.classList.toggle('dark');
            localStorage.setItem('ferret-theme',dark?'dark':'light');
            btn.textContent=dark?'☀':'🌙';
          });
        })();
        """;

    /// <summary>Renders a complete HTML page for the given <paramref name="page"/>.</summary>
    /// <param name="page">The page metadata.</param>
    /// <param name="contentHtml">The rendered HTML content body.</param>
    /// <param name="allPages">All pages in the manual, used to build navigation.</param>
    /// <param name="prev">The previous page, or <see langword="null"/> if none.</param>
    /// <param name="next">The next page, or <see langword="null"/> if none.</param>
    /// <returns>A complete HTML document as a string.</returns>
    public static string Render(
        DocPage page,
        string contentHtml,
        IReadOnlyList<DocPage> allPages,
        DocPage? prev = null,
        DocPage? next = null)
    {
    ...
    private static string BuildNav(IReadOnlyList<DocPage> allPages, string activeSlug)
    {
        ...
        foreach (var (section, pages) in navTree)
        {
            ...
            sb.AppendLine(CultureInfo.InvariantCulture, $"    <span class=\"nav-section-label\">{Escape(section)}</span>");
            sb.AppendLine("    <ul>");
            foreach (var p in pages)
            {
                var active = p.Slug == activeSlug ? " class=\"active\"" : string.Empty;
                sb.AppendLine(CultureInfo.InvariantCulture, $"      <li><a href=\"/manual/{p.Slug}\"{active}>{Escape(p.Title)}</a></li>");
            }
    ...
    private static string BuildFooter(DocPage page, DocPage? prev, DocPage? next)
    {
        ...
        if (prev is not null)
        {
            sb.AppendLine(CultureInfo.InvariantCulture, $"  <a class=\"page-nav-link\" href=\"/manual/{prev.Slug}\">← {Escape(prev.Title)}</a>");
        }
        else
        {
        ...
        sb.AppendLine("  <span class=\"footer-meta\">");
        sb.AppendLine(CultureInfo.InvariantCulture, $"    <a href=\"{editUrl}\" target=\"_blank\" rel=\"noopener\">Edit source</a>");
        sb.AppendLine("    <span class=\"sep\">·</span>");
        sb.AppendLine(CultureInfo.InvariantCulture, $"    <a href=\"{IssueUrl}\" target=\"_blank\" rel=\"noopener\">Report issue</a>");
        sb.AppendLine("    <span class=\"sep\">·</span>");
        sb.AppendLine("    <a href=\"/manual/architecture/index\">Architecture</a>");
        sb.AppendLine("    <span class=\"sep\">·</span>");
        sb.AppendLine("    <a href=\"/manual/reference/cli\">CLI Reference</a>");
        sb.AppendLine("    <span class=\"sep\">·</span>");
        sb.AppendLine("    <span>The Ferret Manual RC1</span>");
        sb.AppendLine("  </span>");

        if (next is not null)
        {
            sb.AppendLine(CultureInfo.InvariantCulture, $"  <a class=\"page-nav-link\" href=\"/manual/{next.Slug}\">{Escape(next.Title)} →</a>");
        }
        else
        {
            sb.AppendLine("  <span></span>");
