...
- **Local-only repos (no remote at all):** fall back to a locally-generated UUID, persisted in that repo's own `.ai/workspace-identity.json` — a new file in the existing per-repo `.ai/` storage area (§19.2), not a new storage area. This identity travels with the repo only if `.ai/` is preserved; a local-only repo cloned to a new machine without its `.ai/` directory gets a new identity, which is correct — an unpushed repo has no server-verifiable durable identity to preserve in the first place.
